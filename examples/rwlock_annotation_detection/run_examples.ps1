...
$expectations = [ordered]@{
    ...
    "callback_rewrapped_fnonce_clean" = $null
    "callback_rewrapped_fnonce_violation" = "read requires no live writer"
...
$summaryOnlyBins = @(
    ...
    "callback_rewrapped_fnonce_clean",
    "callback_rewrapped_fnonce_violation",
    "callback_clean",
    ...
    foreach ($bin in $bins) {
        ...
        $providerBodyEntries = @(
            $output | Where-Object {
                ...
                $_ -match "entered body of .*callback_(accessor|clean|conditional|fnptr_specialization|generic_fnonce|hof_annotated|hof_invoke_twice|loop|nested_hof|returned_guard|rewrapped_fnonce|sequential_counter|specialization).*::(invoke|invoke_from_non_root|invoke_if|invoke_generic|invoke_in_loop|invoke_twice|through_adapter|forward_metadata_mut|with_metadata_after_release|with_metadata_maybe_locked|with_metadata_mut|without_write_held|with_write_held|descriptor_table_mut|annotated_acquire|acquire_once|increment|require|read|\{closure)"
            }
        )
